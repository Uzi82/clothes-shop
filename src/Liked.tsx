...
import { removeLiked } from "./store/likedSlice"
import { Link } from "react-router-dom"

const Liked: React.FC = () => {
    const liked = useAppSelector(state => state.liked.val)
            ...
            <main className='flex justify-center'>
                    {
                                        ...
                                        <div key={el._id} className='widthCard h-96 p-5 rounded-lg bg-indigo-500'>
                                            <div className='flex w-full h-full flex-col aling-center justify-between'>
                                                ...
                                                <div className='flex flex-row justify-between px-3'>
                                                    <button onClick={() => dispatch(removeLiked(el._id))} className='buttons'>
                                                        Delete
                                                    </button>
                                                </div>
                                            </div>
                        ...
                        :
                            <h1 className="width text-center my-10 text-5xl text-indigo-700">No liked yet</h1>
                    }
            </main>
        </>
    )
